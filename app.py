...
def edit_doc(id):
    ...
    if request.method == 'POST' and form.validate():
        ...
        if dbcontroller.edit_doc(id, form.title.data, form.content.data, form.is_private.data):
            dbcontroller.edit_hist(id, doc['title'], session['username'], doc['content'], doc['modify_date'])
...
def doc_hist_list(id):
    doc, empty = dbcontroller.get_hist_list(id, session['username'])

    if doc["history"] or empty:
        if doc["history"]:
            return render_template('doc_hist_list.html', docs=doc["history"], doc_id=doc["_id"])
        else:
...
@app.route('/doc_hist/<string:doc_id>/<string:record_id>', methods=['GET'])
@is_logged_in
def doc_hist(doc_id, record_id):
    # User logged in
    try:
        if session['logged_in'] == True:
            doc = dbcontroller.get_doc_hist_by_id(doc_id, record_id, session['username'])
            # Have access to doc
            if doc:
                return render_template('doc_hist.html', doc=doc, doc_id=doc_id)
            # No access to doc
            else:
                flash("You don't have access to this document!", 'danger')
                return redirect(url_for('home'))
